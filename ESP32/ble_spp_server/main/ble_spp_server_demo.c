        ...
        if (xQueueReceive(spp_uart_queue, (void * )&event, (portTickType)portMAX_DELAY)) {
            ...
            case UART_DATA:
                if ((event.size)&&(is_connected)) {
                    ...
                    if (event.size)
                    {
                        ESP_LOGW(GATTS_TABLE_TAG, "get uart (%d), send ble ",event.size);
                    }
...
{
    esp_err_t err;
    ESP_LOGW(GATTS_TABLE_TAG, "GAP_EVT, event %d\n", event);
...
static void gatts_profile_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    switch (event) {
    	...
    	case ESP_GATTS_READ_EVT:
            ...
            ESP_LOGI(GATTS_TABLE_TAG, "tx-rx test tx callback ");        // 等客户端来取
            if(res == SPP_IDX_SPP_STATUS_VAL){
                //TODO:client read the status characteristic
                ESP_LOGI(GATTS_TABLE_TAG, "tx-rx test 1 ");
            }
       	 break;
    	case ESP_GATTS_WRITE_EVT: 
        {
            ...
            if(p_data->write.is_prep == false){
                ESP_LOGW(GATTS_TABLE_TAG, "ESP_GATTS_WRITE_EVT : handle = %d ", res);
                if(res == SPP_IDX_SPP_COMMAND_VAL){
                    ...
                    ESP_LOGW(GATTS_TABLE_TAG, "tx-rx test rx (1)");
                    memset(spp_cmd_buff,0x0,(spp_mtu_size - 3));
                    memcpy(spp_cmd_buff,p_data->write.value,p_data->write.len);
                    xQueueSend(cmd_cmd_queue,&spp_cmd_buff,10/portTICK_PERIOD_MS);
                }
                else if(res == SPP_IDX_SPP_DATA_NTF_CFG){
                    ESP_LOGW(GATTS_TABLE_TAG, " read ");
                    if((p_data->write.len == 2)&&(p_data->write.value[0] == 0x01)&&(p_data->write.value[1] == 0x00)){
                        ESP_LOGW(GATTS_TABLE_TAG, "open send data");
                        enable_data_ntf = true;
                    }else if((p_data->write.len == 2)&&(p_data->write.value[0] == 0x00)&&(p_data->write.value[1] == 0x00)){
                        ESP_LOGW(GATTS_TABLE_TAG, "off send data");
...
#else
                    ESP_LOGW(GATTS_TABLE_TAG, "tx-rx test rx (2) default");
                    uart_write_bytes(UART_NUM_0, (char *)(p_data->write.value), p_data->write.len);
#endif
                }else{
                    //TODO:
                    ESP_LOGW(GATTS_TABLE_TAG, "tx-rx test rx (3) void ");
                }
            }
            else if((p_data->write.is_prep == true)&&(res == SPP_IDX_SPP_DATA_RECV_VAL))
            {
                ESP_LOGW(GATTS_TABLE_TAG, " printf test 3 -->");
    	...
    	case ESP_GATTS_CONF_EVT:
            ESP_LOGI(GATTS_TABLE_TAG, " test to send ");
    	    break;
    	...
    	case ESP_GATTS_CONNECT_EVT:
            ESP_LOGW(GATTS_TABLE_TAG, " ble CONNECT ");
    	...
    	case ESP_GATTS_DISCONNECT_EVT:
            ESP_LOGW(GATTS_TABLE_TAG, " ble DISCONNECT ");
    	    is_connected = false;
    	    enable_data_ntf = false;
#ifdef SUPPORT_HEARTBEAT
    	    enable_heart_ntf = false;
